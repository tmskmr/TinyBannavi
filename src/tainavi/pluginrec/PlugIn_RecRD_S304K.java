...
public class PlugIn_RecRD_S304K extends HDDRecorderUtils implements HDDRecorder,Cloneable {

	public PlugIn_RecRD_S304K clone() {
		return (PlugIn_RecRD_S304K) super.clone();
	}
	...
	// 種族の特性
	public String getRecorderId() { return "VARDIA RD-S304K"; }
	...
	public boolean GetRdReserve(boolean force)
	{
		...
		// (1)録画設定の取得
		{
			...
			reqGET("http://"+getIPAddr()+":"+getPortNo()+"/program/b_proginfo.htm?bExec=1",null);
		}
		{
		...
		}
		{
			...
			*/
			...
			// (1-1)画質設定
			{
				...
				t = new TextValueSet();
				t.setText("[TSE] AT 4.7GB");
				t.setValue("2:4");
				vrate.add(t);
				t = new TextValueSet();
				t.setText("[TSE] AT 9.4GB");
				t.setValue("2:5");
				vrate.add(t);
				t = new TextValueSet();
				t.setText("[TSE] AT 8.5GB");
				t.setValue("2:7");
				vrate.add(t);
		...
		*/
		...
		if ( ma.find() ) {
			...
			if (mb.find()) {
				errmsg = mb.group(1);
				System.out.printf("\n<<< Message from RD >>> \"%s\"\n\n", errmsg);
				Matcher mc = Pattern.compile("(正常に登録できました。|予約時間が重複しています。|Ｗ録の振り替えをおこないました)").matcher(errmsg);
				if ( ! mc.find() ) {
					return(false);
				}
			}
