...
public class PlugIn_RecRD_XS57 extends HDDRecorderUtils implements HDDRecorder,Cloneable {

	public PlugIn_RecRD_XS57 clone() {
		return (PlugIn_RecRD_XS57) super.clone();
	}
	...
	// 種族の特性
	public String getRecorderId() { return "RD-XS57"; }
	...
	public boolean GetRdReserve(boolean force)
	{
		...
		}
		{
			...
			mb = Pattern.compile("<!--[\\s\\S]*?if\\( double_encode_flg == 1 \\) \\{([\\s\\S]*?)\\}\\s*\\}\\s*-->").matcher(res);
			if (mb.find()) {
				Matcher mc = Pattern.compile("if \\( encoder == 1 \\) \\{([\\s\\S]*?)\\}").matcher(mb.group(1));
				if (mc.find()) {
					Matcher md = Pattern.compile("name=enc_type value=\\\\\"(.+?)\\\\\"[\\s\\S]+?toru_(.+?)\\.gif").matcher(mc.group(1));
					while (md.find()) {
						TextValueSet t = new TextValueSet();
						t.setText(md.group(2));
						t.setValue(md.group(1));
						encoder.add(t);
					}
				}
			}
			...
			// (1-5)DVD互換モード
			setSettingEtc(dvdcompat,"dvdr",0,res);
			...
			setSettingEtc(mschapter,"dvdchapter",0,res);
			TVSsave(mschapter, msChapterTFile);

			setSettingEtc(mvchapter,"mvchapter",0,res);
			...
			for ( String typ : new String[] { "uvd","bsd","csd","uva","bsa","l1","l2","l3" } ) {
				...
				if ( mc.find() && md.find() ) {
					...
					while ( me.find() && mf.find() ) {
						...
						t.setValue((typ.matches("^l[123]")?("L"):(""))+mf.group(1));
						chvalue.add(t);
		...
		// 情報ブロックをとりだし…
		{
			...
			while ( ma.find() ) {
				...
				String enc = value2text(encoder, d[3]);
				if (enc.length() > 0) {
					entry.setTuner(enc);
				}
				else {
				...
				//entry.setRec_audio(_recstr2audio(d[10]));
...
//			"audio_es",
	...
	private Hashtable<String, String> modPostdata(ReserveList r) {
		...
		try {
			...
			//System.out.println(r.getCh_name()+"; "+channel);
			...
			if (chtype.size() > 0) {
				...
				if (ma.find()) {
					if (channel.matches("^L.*")) {
						newdat.put("broadcast", String.valueOf(Integer.valueOf(channel.substring(1))+1));	// ライン入力ＡＢＣ
					}
					else if ((ma.group(1)+ma.group(2)).equals("BS")) {
						newdat.put("broadcast","1");		// BSデジタル
					}
					else {
						newdat.put("broadcast","0");		// 地上アナログ
					}
					
					newdat.put("channel_list", channel.replaceFirst("^L", ""));
					newdat.put("channel_no", channel.replaceFirst("^L", ""));
				}
			}
	...
	private void setSettingEtc(ArrayList<TextValueSet> tvs, String key, int typ, String res) {
		...
		if (mc.find() && md.find()) {
			...
			while (me.find() && mf.find()) {
				TextValueSet t = new TextValueSet();
				if (key.equals("vrate")) {
					t.setText(me.group(1).replaceAll(" ", ""));
				}
				else {
					t.setText(me.group(1));
				}
