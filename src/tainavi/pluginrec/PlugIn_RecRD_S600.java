...
public class PlugIn_RecRD_S600 extends HDDRecorderUtils implements HDDRecorder,Cloneable {

	@Override
	public PlugIn_RecRD_S600 clone() {
		return (PlugIn_RecRD_S600) super.clone();
	}
	...
	// 種族の特性
	@Override
	public String getRecorderId() { return "VARDIA RD-S600"; }
	@Override
...
//			"audio_es",
	...
	private Hashtable<String, String> modPostdata(ReserveList r) {
		...
		try {
			...
			if (getDebug()) System.err.println("[DEBUG] Exec="+newdat.get("bExec"));
			...
			if (getDebug()) System.err.println("[DEBUG] title_name="+newdat.get("title_name"));
			...
			if (getDebug()) System.err.println("[DEBUG] folder="+newdat.get("folder"));
			
			// ジャンル
			newdat.put("genre", _recstr2genre(r.getRec_genre()));
			
			if (getDebug()) System.err.println("[DEBUG] genre="+newdat.get("genre"));
			
			// DVD互換モード
			newdat.put("dvdr", text2value(dvdcompat, r.getRec_dvdcompat()));
			
			if (getDebug()) System.err.println("[DEBUG] dvdr="+newdat.get("dvdr"));
			...
			if (getDebug()) System.err.println("[DEBUG] channel_no="+newdat.get("channel_no"));
			
			// 録画レート
			newdat.put("enc_type", text2value(encoder, r.getTuner()));
			newdat.put("vrate", text2value(vrate, r.getRec_mode()));
			
			if (getDebug()) System.err.println("[DEBUG] vrate="+newdat.get("vrate"));
			
			// 録音レートの指定がねえな…
			newdat.put("amode", text2value(arate, r.getRec_audio()));
			
			if (getDebug()) System.err.println("[DEBUG] arate="+newdat.get("arate"));
			...
			if (getDebug()) System.err.println("[DEBUG] date="+newdat.get("date"));
