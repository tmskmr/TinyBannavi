 ...
 * @version x.xx.xx
 */
public class PlugIn_RecDBR_M2008 extends HDDRecorderUtils implements HDDRecorder,Cloneable {
	public PlugIn_RecDBR_M2008() {
		super();
		this.setTunerNum(3);
		setSettingFixed();
	}

	public PlugIn_RecDBR_M2008 clone() {
		return (PlugIn_RecDBR_M2008) super.clone();
	}
	...
	// 種族の特性
	@Override
	public String getRecorderId() { return "REGZA DBR-M2008"; }
	@Override
...
//		String header = d[0];
		...
		while ( ma.find() ) {
			...
			String recording = ma.group(13);
			String mochidashi = ma.group(15);
			...
			entry.setRec_portable(portable_name);
	...
	protected void setReserveBasicInfo(ReserveList entry, String exec, String option, String eventname,
		...
		// チャンネル
		entry.setChannel(getFormalChFromNetworkAndCh(network, ch));
	...
	/*
	 * network と ch から 正式なチャンネルIDを取得する
	 */
	protected String getFormalChFromNetworkAndCh(String network, String ch){
		// チャンネル
		switch(network){
		case NETWORK_UVD:
			break;
...
//			String autorecflag = ma.group(10);
			...
			// チャンネルを変換する
			ch = getFormalChFromNetworkAndCh(network, ch);
			...
			// 録画モード
			String recmode_name = value2text(vrate, recmode);
			entry.setRec_mode(recmode_name);
...
//			String header = d[0];
		...
		Matcher ma = Pattern.compile("\\{" +
				...
				"\"dlnaObjectID\":\"([^\"]*)\"," +	// 16
				"\"rating\":(\\d+)," +				// 17
				"\"recording\":(false|true)," +		// 18
				"\"chapter\":\\[([^\\]]*)\\]," +	// 19
				"\"autodel\":(false|true)," +		// 20
				"\"protection\":(false|true)," +	// 21
				"\"mochidashiContentId\":\"([^\"]*)\"," +	// 22
				"\"resumeTime\":(\\d+)," +			// 23
				"\"shortTimeInfo\":\\[([^\\]]*)\\]," +// 24
				"\"detailDescription\":\"([^\"]*)\"," +		// 25
				"\"profiles\":\\[([^\\]]*)\\]," +		// 26
				"\"mochidashiState\":(\\d+)," +			// 27
				"\"defective\":(false|true)," +	// 28
				"\"portableEnable\":(false|true)" +	// 29
				"\\}")
				.matcher(response);
		...
		String folders = ma.group(4);
		String ch = ma.group(9);
		String recmode = ma.group(11);
		String genres = ma.group(12);
		String datetime = ma.group(13);
		String duration = ma.group(14);
		String copycount = ma.group(15);
		String dlnaObjectID = ma.group(16);
		Boolean recording = ma.group(18).equals("true");
//		String recording  = ma.group(12);
		String chapter = ma.group(19);
		String portableEnable = ma.group(29);
