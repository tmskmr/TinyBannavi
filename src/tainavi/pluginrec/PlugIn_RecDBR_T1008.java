...
/**
 * REGZA DBR-T1008用のレコーダプラグインです。
 * @author original:tmskmr
 * @version x.xx.xx
 */
public class PlugIn_RecDBR_T1008 extends HDDRecorderUtils implements HDDRecorder,Cloneable {
	public PlugIn_RecDBR_T1008() {
		super();
		this.setTunerNum(3);
		setSettingFixed();
	}

	public PlugIn_RecDBR_T1008 clone() {
		return (PlugIn_RecDBR_T1008) super.clone();
	}
	...
	// 種族の特性
	@Override
	public String getRecorderId() { return "REGZA DBR-T1008"; }
	@Override
...
//		String header = d[0];
	...
	protected boolean loadDialogInitData( String id) {
		...
		String[] d = reqGET("http://"+getIPAddr()+":"+getPortNo()+"/torunavi/LoadDialogInitializationData.php?schId=" + id, null, thisEncoding);
//		String header = d[0];
		String response= d[1];
...
//			String header = d[0];
		...
		Matcher ma = Pattern.compile("\\{" +
				...
				"\"datetime\":(\\d+)," +			// 8
				"\"duration\":(\\d+)," +			// 9
				"\"newflag\":(false|true)," +		// 10
				"\"autorecflag\":(false|true)," +	// 11
				"\"autodelflag\":(false|true)," +	// 12
				"\"protection\":(false|true)," +	// 13
				"\"mochidashiState\":(\\d+)," +		// 14
				"\"recmode\":(\\d+)," +				// 15
				"\"recording\":(false|true)\\}")	// 16
				.matcher(response);
		...
		while ( ma.find() ) {
			...
			String newflag = ma.group(10);
			boolean autorecflag = ma.group(11).equals("true");
			boolean autodelflag = ma.group(12).equals("true");
			boolean protection = ma.group(13).equals("true");
			String mochidashiState = ma.group(14);
			String recmode = ma.group(15);
			boolean recording = ma.group(16).equals("true");
			...
			setTitleBasicInfo(entry, titlename, network, ch, datetime, duration, device_id, folders, genres);
	 ...
	 *  タイトルの基本情報をセットする
	 */
	protected void setTitleBasicInfo(TitleInfo entry, String titlename, String network,
...
//			String header = d[0];
		...
		Matcher ma = Pattern.compile("\\{" +
				...
				"\"dlnaObjectID\":\"([^\"]*)\"," +	// 15
				"\"rating\":(\\d+)," +				// 16
				"\"recording\":(false|true)," +		// 17
				"\"chapter\":\\[([^\\]]*)\\]," +	// 18
				"\"autodel\":(false|true)," +		// 19
				"\"protection\":(false|true)," +	// 20
				"\"mochidashiContentId\":\"([^\"]+)\"," +	// 21
				"\"resumeTime\":(\\d+)," +			// 22
				"\"shortTimeInfo\":\\[([^\\]]*)\\]," +	// 23
				"\"detailDescription\":\"([^\"]*)\"," +	// 24
				"\"profiles\":\\[([^\\]]*)\\]," +	// 25
				"\"mochidashiState\":(\\d+)," +		// 26
				"\"defective\":(false|true)\\}")	// 27
				.matcher(response);
		...
		String network = ma.group(5);
		String ch = ma.group(9);
		String recmode = ma.group(10);
		String genres = ma.group(11);
		String datetime = ma.group(12);
		String duration = ma.group(13);
		String copycount = ma.group(14);
		String dlnaObjectID = ma.group(15);
		Boolean recording = ma.group(17).equals("true");
		String chapter = ma.group(18);

		// 基本情報をセットする
		setTitleBasicInfo(t, titlename, network, ch, datetime, duration, device_id, folders, genres);
		...
		// 録画モード
//		String recmode_name = value2text(vrate, recmode);
		Matcher mb = Pattern.compile("\"id\":(\\d+),\"name\":\"([^\"]+)\"").matcher(recmode);
		if (mb.find())
			t.setRec_mode("[" + mb.group(2) + "]");
	...
	private boolean notifyTitleEdit(String devid, String ttlid) {
		...
		// RDへの情報作成
		String pstr = "drive_id=" + devid + "&title_id=" + ttlid;
...
//		setSettingSelect(repeats, discs, folder, vrate);
	...
	protected boolean setSettingDevice() {
		...
		Matcher ma = Pattern.compile("\\{" +
				...
				"\"protected\":(true|false)," +		// 9
				"\"mounted\":(true|false)," +		// 10
				"\"ready\":(true|false)," +			// 11
				"\"pin_setting\":(true|false)," +	// 12
				"\"formatType\":(\\d+)\\}")			// 13
				.matcher(res);
		...
		while ( ma.find() ) {
			...
			boolean isprotected = ma.group(9).equals("true");
			boolean mounted = ma.group(10).equals("true");
			boolean ready = ma.group(11).equals("true");
			boolean pin_setting = ma.group(12).equals("true");
	...
	protected boolean setSettingFolders() {
		...
		for (int n=0; n<tvsD.size(); n++){
			...
			String pstr = "drive=" + drive_id;

			// おまじない
			Authenticator.setDefault(new MyAuthenticator(getUser(), getPasswd()));

			// RDへ情報送信
			System.out.println(MSGID+"レコーダーにフォルダー一覧を要求します:" + pstr);
			String[] d = reqGET("http://"+getIPAddr()+":"+getPortNo()+"/folderset/LoadFolderList.php?" + pstr, null, thisEncoding);
			String res = d[1];

			if (res == null) {
				System.out.println(ERRID+"レコーダーからのレスポンスがありません.");
				errmsg = ERRID+ERRMSG_NORESPONSE;
				folder = tvsF;
				return (false);
			}

			System.out.println(MSGID+"レコーダーからのレスポンスは以下の通りです:" + res);
			Matcher ma = Pattern.compile("\\{" +
					"\"folder_id\":(\\d+)," +		// 1
					"\"foldername\":\"([^\"]+)\"\\}")		// 2
					.matcher(res);
			...
			while ( ma.find() ) {
				...
				System.out.println(MSGID+"フォルダ(" + folder_id + "," + folder_name + ")を追加します.");
				t.setText(folder_name);
