...
public class Ts {
	...
	private static final int RET_GOTO_RETRY = -2;
	private static final int RET_RETURN_NULL = -3;
	
	private static int rcount  = 0;
	private static int ridx = -1;
	
	//
	public SECcache readTS(PushbackInputStream in, ArrayList<SECcache> secs, int size) {
	...
	//retry:
			...
			while ( true ) {
				...
				else if ( ret == RET_GOTO_RETRY ) {
					continue;
				}
				
				ret = foo2(in,secs,size);
				if ( ret >= 0 ) {
					return secs.get(ret);
				}
				else if ( ret == RET_RETURN_NULL ) {
					return null;
				}
				else if ( ret == RET_GOTO_RETRY ) {
					continue;
	...
	//
	private boolean skip2syncbyte(PushbackInputStream in) throws IOException {
		int inchar;
		
		// syncバイトまで読み飛ばし
		if ( rcount == 0 ) {
			do {
				if ( (inchar = in.read()) != EOF ) {
					if ( (inchar & 0xFF) == 0x47 ) {
						in.unread(inchar);
						break;
					}
	...
	private int foo1(ArrayList<SECcache> secs) {
		
		if ( ridx >= 0 && secs.get(ridx).cont != 0 ) {
			// バッファチェック
			if ( (secs.get(ridx).cur.payload[secs.get(ridx).curlen] & 0xFF) == 0xFF ) {
				secs.get(ridx).cont = 0;
				secs.get(ridx).seclen = 0;
				secs.get(ridx).setlen = 0;
				secs.get(ridx).curlen = 0;
			}
			else {
				int len = secs.get(ridx).cur.payloadlen - secs.get(ridx).curlen;
				if ( len == 0 ) {
					secs.get(ridx).cont = 0;
					secs.get(ridx).seclen = 0;
					secs.get(ridx).setlen = 0;
					...
					*/
					...
					int[] boff = {12};
					try { 
					secs.get(ridx).seclen = Util.getBit(secs.get(ridx).cur.payload, secs.get(ridx).curlen, boff, 12) + 3; // ヘッダ
					}
					catch ( Exception e ) {
						System.err.println(secs.get(ridx).curlen);
						e.printStackTrace();
					}
					...
					*/
					...
					// TSデータ長-設定済みデータ長
					if ( secs.get(ridx).seclen > len ) {
						System.arraycopy(secs.get(ridx).cur.payload, secs.get(ridx).curlen, secs.get(ridx).buf, 0, len);
						//memcpy(secs[ridx].buf, &secs[ridx].cur.payload[secs[ridx].curlen], len);
						secs.get(ridx).setlen = len;
						secs.get(ridx).curlen = 0;
						secs.get(ridx).cont   = 1;
						// 次のレコード読み込み
					}
					else {
						System.arraycopy(secs.get(ridx).cur.payload, secs.get(ridx).curlen, secs.get(ridx).buf, 0, secs.get(ridx).seclen);
						//memcpy(secs[ridx].buf, &secs[ridx].cur.payload[secs[ridx].curlen], secs[ridx].seclen);
						secs.get(ridx).setlen = secs.get(ridx).seclen;
						secs.get(ridx).curlen += secs.get(ridx).seclen;
						secs.get(ridx).cont = 1;

						// CRCのチェック
						if ( checkcrc(secs.get(ridx)) ) {
							return ridx; /* 戻る */
						}
						return RET_GOTO_RETRY; /* もう一回 */
					}
				}
			}
		}
	...
	private int foo2(PushbackInputStream in, ArrayList<SECcache> secs, int size) throws IOException {
		TSpacket pk = new TSpacket();
		...
		int roffset = 0;
		while ( true ) {
			if ( in.read(buf, roffset, 188-roffset) < 1 ) {
				/* 残りの処理? */
				return RET_RETURN_NULL;
			}
			roffset = 0;
			rcount++;
			...
			if ( (buf[0] & 0xFF) != 0x47) {
				...
				if ( i < 188 ) {
					...
					roffset = i;
					int[] tmp = new int[188-i];
					System.arraycopy(buf, i, tmp, 0, 188-i);
					System.arraycopy(tmp, 0, buf, 0, 188-i);
			...
			*/
			...
			boff[0] = 0;
			pk.sync = Util.getBit(buf, 0, boff, 8);
			pk.transport_error_indicator = Util.getBit(buf, 0, boff, 1);
			pk.payload_unit_start_indicator = Util.getBit(buf, 0, boff, 1);
			pk.transport_priority = Util.getBit(buf, 0, boff, 1);
			pk.pid = Util.getBit(buf, 0, boff, 13);
			pk.transport_scrambling_control = Util.getBit(buf, 0, boff, 2);
			pk.adaptation_field_control = Util.getBit(buf, 0, boff, 2);
			pk.continuity_counter = Util.getBit(buf, 0, boff, 4);
			...
			*/
			...
			Arrays.fill(pk.payload, (byte) 0xFF);
			System.arraycopy(buf, payptr, pk.payload, 0, pk.payloadlen);
			...
			*/
			...
			for ( int i=0; i<size; i++ ) {
				if ( secs.get(i).pid == pk.pid ) {
					secs.get(i).cur = pk;
					/* 途中処理中か最初か? */
					if ( secs.get(i).cont == 0 ) {
						/* 最初 セクション長を調べる */
						boff[0]= 12;
						secs.get(i).seclen = Util.getBit(secs.get(i).cur.payload, 0, boff, 12) + 3; // ヘッダ;
						/*
						  if(secs.get(i).seclen == 2334) {
						  printf("aa");
						  }
						*/

						if ( secs.get(i).seclen > secs.get(i).cur.payloadlen ) {
							System.arraycopy(secs.get(i).cur.payload, 0, secs.get(i).buf, 0, secs.get(i).cur.payloadlen);
							//memcpy(secs.get(i).buf, secs.get(i).cur.payload, secs.get(i).cur.payloadlen);
							secs.get(i).setlen = secs.get(i).cur.payloadlen;
							secs.get(i).cont = 1;
							continue;
						}
						System.arraycopy(secs.get(i).cur.payload, 0, secs.get(i).buf, 0, secs.get(i).seclen);
						//memcpy(secs.get(i).buf, secs.get(i).cur.payload, secs.get(i).seclen);
						secs.get(i).setlen = secs.get(i).seclen;
						secs.get(i).curlen = secs.get(i).seclen;
						secs.get(i).cont = 1;
						ridx = i;
						/* CRCのチェック */
						if ( checkcrc(secs.get(ridx)) ) {
						...
						return RET_GOTO_RETRY; /* 残り処理へ */
					}
					/* セクション長-設定済み長 */
					len = secs.get(i).seclen - secs.get(i).setlen;
					if ( len > secs.get(i).cur.payloadlen ) {
						/* 全体転送 */
						System.arraycopy(secs.get(i).cur.payload, 0, secs.get(i).buf, secs.get(i).setlen, secs.get(i).cur.payloadlen);
						//memcpy(&secs.get(i).buf[secs.get(i).setlen], secs.get(i).cur.payload, secs.get(i).cur.payloadlen);
						secs.get(i).setlen += secs.get(i).cur.payloadlen;
						continue;
					}
					/* セクション長の残りを設定 */
					System.arraycopy(secs.get(i).cur.payload, 0, secs.get(i).buf, secs.get(i).setlen, len);
					//memcpy(&secs.get(i).buf[secs.get(i).setlen], secs.get(i).cur.payload, len);
					secs.get(i).setlen  = secs.get(i).seclen;
					secs.get(i).curlen += len;
					secs.get(i).cont    = 1;
					ridx = i;
					/* CRCのチェック */
					if( checkcrc(secs.get(ridx)) ) {
	...
	private int CalcCrc(int crc, byte[] buf, int len) {
		int c = crc;
		int n;
	...
	//
	private boolean checkcrc(SECcache secs) {
		/* セクションの終りに置かれる4バイトのCRC32は、
		   CRC計算の結果0になるように設定される。
		   値が発生した場合は、エラーなので対象外にする */
		if ( CalcCrc(0xFFFFFFFF, secs.buf, secs.seclen) != 0 ) {
			if (Util.debug) System.err.println(String.format("tblid:0x%x CRC error", secs.buf[0]));
			return false;
		}
		return true;
	}
